...
def timedetect(source_path, zone_configuration_path, violation_time, confidence, language: str, analysis_path: str):
    ...
    # Try to load the custom model, fallback to available models
    # Get absolute paths to ensure we find the files
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    weights_dir = os.path.join(project_root, "weights")
    
    model_paths = [
        os.path.join(weights_dir, "best_ps3.pt"),  # Custom trained model (preferred)
        os.path.join(weights_dir, "best.pt"),      # Alternative custom model
        os.path.join(weights_dir, "yolov8n.pt")    # Default YOLOv8 model
    ]
    ...
    if model is None:
        error_msg = f"❌ No suitable model found! Tried: {', '.join(model_paths)}"
        st.error(error_msg)
        st.info("💡 **Please ensure one of the following model files exists:**")
        st.markdown("""
        - `weights/best_ps3.pt` (preferred - custom trained model)
        - `weights/best.pt` (alternative custom model)
        - `weights/yolov8n.pt` (default YOLOv8 model)
        """)
        return
    
    if model_path != model_paths[0]:
        st.info(f"ℹ️ Using model: {model_path} (custom model not found, using fallback)")
    ...
    # Create terminate button outside the loop to avoid duplicate key errors
    col1, col2 = st.columns([3, 1])
    with col2:
        terminate_button = st.button("⏹️ Terminate", key="terminate_timedetect", type="secondary")
        if terminate_button:
...
def livedetection(source_url: str, violation_time: int, zone_configuration_path: str, confidence: float, analysis_path: str):
    ...
    # Try to load model with fallback (fix Windows/Unix path separator)
    # Get absolute paths to ensure we find the files
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(script_dir)
    weights_dir = os.path.join(project_root, "weights")
    ...
    if model is None:
        st.error(f"❌ No suitable model found! Tried: {', '.join(model_paths)}")
        st.info("💡 Please ensure `weights/yolov8n.pt` or `weights/best.pt` exists.")
        return
    terminate = st.button("Terminate Stream", key="terminate_livedetection")
